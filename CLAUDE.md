...
| `src/editor/serialization/` | `HTMLSerializer`, `JSONSerializer`, `MarkdownSerializer` — serialize/deserialize the document model. `HTMLSerializer` handles all block types (including table, check_list, image with attrs) and all marks (including font_size, font_family, text_color, highlight with color) in both directions. Used by source editing mode and paste. |
| `src/components/Editor.tsx` | React wrapper — owns the `contentEditable` div, wires `onBeforeInput`, `onKeyDown`, `onFocus`, `onMouseDown`, click, and paste handlers. Uses `useLayoutEffect` (not `useEffect`) for DOM sync + cursor restore. `onMouseDown` intercepts clicks on `[data-check-path]` inputs to toggle `check_list_item`, clicks on `[data-image-path]` to select images, and mousedown on resize handles for column/image resize. Accepts optional `onUploadImage?: (file: File) => Promise<string>` prop for custom upload. **Source editing mode**: `isSourceMode` state; when active, replaces the `contentEditable` div with a `<textarea>` containing pretty-printed HTML. Toggling back deserializes the textarea HTML via `htmlSerializer.deserialize` and dispatches `replace_doc`. |
...
| `src/components/toolbar/SourceEditingButton.tsx` | Toolbar button (HTML icon) that toggles source editing mode. Stateless — calls `onToggle` prop. |
