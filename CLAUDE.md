...
| `src/components/Editor.tsx` | React wrapper — owns the `contentEditable` div, wires `onBeforeInput`, `onKeyDown`, `onFocus`, `onMouseDown`, click, and paste handlers. Uses `useLayoutEffect` (not `useEffect`) for DOM sync + cursor restore. `onMouseDown` intercepts clicks on `[data-check-path]` inputs to toggle `check_list_item` checked state without moving the cursor. |
